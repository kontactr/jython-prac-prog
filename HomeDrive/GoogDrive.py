...
def goog_drive_auth(name):
    ...
    if credentials == False:
        ...
        return render_template("DriveTemp.html",Drive_name = "Google", path=session[request.environ['REMOTE_ADDR'] + 'user_dir'])
...
def oauth2callback():
    ...
    if 'code' not in flask.request.args:
        ...
        open('Credentials\\'+session[request.environ['REMOTE_ADDR'] + 'session_user']+'credentials.json', 'w').write(credentials.to_json())  # write access token to credentials.json locally
        return flask.redirect(flask.url_for('goog_drive_auth',name=session[request.environ['REMOTE_ADDR'] + 'session_user']))


def get_credentials():
    #file_name = session[request.environ['REMOTE_ADDR'] + 'username']+'credentials.json'
    file_name = 'Credentials\\'+session[request.environ['REMOTE_ADDR'] + 'session_user']+'credentials.json'
